...
test("renders the live desktop trading workstation", async ({ page }) => {
  ...
  await expect(page.getByLabel("形态图解")).toBeVisible();
  await expect(page.getByLabel("形态图解").getByText("推动3")).toBeVisible();
  ...
  if (runState === "已完成") {
    ...
    expect(postRunLayout.quoteHeight).toBeGreaterThan(132);
    expect(postRunLayout.chartStageHeight).toBeGreaterThan(620);
